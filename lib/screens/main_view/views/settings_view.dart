...
class _SettingsViewState extends State<SettingsView> {
  ...
  String _getFormattedPeriodEnd(DashboardViewmodel viewModel) {
    final rawDate = viewModel.currentSubscriptionResponse?.data?.currentPeriodEnd;
  ...
  Widget build(BuildContext context) {
    return Consumer<DashboardViewmodel>(
      builder: (context, viewModel, child) {
        final isUserSignedIn = _isSignedIn || (viewModel.currentUser?.user != null);
                    ...
                    PopupMenuButton<String>(
                      ...
                      onSelected: (value) {
                        if (value == 'delete') {
                          ...
                          Future.microtask(() async {
                            try {
                              final authRepo = serviceLocator<AuthenticationRepo>();
                              await authRepo.logout();
                            } catch (_) {}
                            final secureStorageService = serviceLocator<SecureStorageService>();
                            await secureStorageService.delete(key: StorageKeys.accessToken);
                            if (context.mounted) {
                      ...
                      itemBuilder: (context) => isUserSignedIn 
                        ? [
                            PopupMenuItem(
                              value: 'logout',
                              child: Text("Log Out".tr(context, listen: false)),
                            ),
                            PopupMenuItem(
                              value: 'delete',
                              child: Text("Delete Account".tr(context, listen: false)),
                            ),
                          ]
                        : [
                            PopupMenuItem(
                              value: 'signup',
                              child: Text("Sign Up".tr(context, listen: false)),
                            ),
                          ],
                    )
                  ],
                      ...
                      children: [
                        ...
                        RichText(
                          text: TextSpan(
                            ...
                            children: [
                              TextSpan(text: "Your account will be permanently deleted in ".tr(context)),
                              TextSpan(
                                text: "${viewModel.deletionDaysRemaining} days.".tr(context),
